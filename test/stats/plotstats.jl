...
function pltinvslvmu(str::String)
  ...
  for i in 1: length(d["nc2"])
    ...
    ax[:plot](d["sl"][i,:], d["mu"], marker[i], label = "\$||C_{2}||\$ =$(round(d["nc2"][i], 1))", color = col[i], markersize=2.5)
